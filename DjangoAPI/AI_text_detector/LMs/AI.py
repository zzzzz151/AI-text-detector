...
class TestLanguageModel:
    ...
    def load_dataset(self, dataset)->list:
        print("Loading dataset...")
        pf = pd.read_csv(dataset)
    ...
    def test_model(self, dataset_name):
        ...
        for string, is_synthetic in dataset:
            datapoint_evaluation_time_started = time.time()
            probability = round(self.model.model(string) * 100)
            results.append((probability, is_synthetic, len(string), time.time() - datapoint_evaluation_time_started))
        dataset_evaluation_time_taken = time.time() - dataset_evaluation_time_started

        probability_when_human = [probability for probability, is_synthetic, _, _ in results if is_synthetic == 0]
        average_when_human = sum(probability_when_human) / len(probability_when_human)

        probability_when_AI = [probability for probability, is_synthetic, _, _ in results if is_synthetic == 1]
        average_when_AI = sum(probability_when_AI) / len(probability_when_AI)
